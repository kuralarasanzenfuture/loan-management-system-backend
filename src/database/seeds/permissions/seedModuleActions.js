...
const MODULE_ACTIONS_CONFIG = {
  ...
  MOD_ANALYTICS: VIEW_ONLY_ACTION,

  // --- Customer Management ---
  MOD_CUSTOMERS: STANDARD_CRUD_ACTIONS,
  ...
  MOD_BANK_TRANSACTIONS: [
    ...
    },
    {
      action_code: "REVERSE",
      action_name: "Reverse",
      description: "Reverse bank transaction",
    },
  ],
  MOD_ASSET_CATEGORIES: STANDARD_CRUD_ACTIONS,
  MOD_ASSETS: STANDARD_CRUD_ACTIONS,
...
export const SeedModuleActionsTable = async () => {
  ...
  try {
    ...
    console.log(
      ` ✅ Successfully seeded ${totalInserted} module actions across all active modules!`,
    );
  } catch (error) {
    await connection.rollback();
    console.error("❌ Error seeding module_actions table:", error.message);
